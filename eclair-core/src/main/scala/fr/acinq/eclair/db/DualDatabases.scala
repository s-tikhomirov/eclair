...
import fr.acinq.eclair.channel.{ChannelErrorOccurred, HasCommitments, HtlcSettlementCommand, NetworkFeePaid}
 ...
 */
case class DualDatabases(sqlite: SqliteDatabases, postgres: PostgresDatabases) extends Databases {
  ...
  override val pendingRelay: PendingRelayDb = DualPendingRelayDb(sqlite.pendingRelay, postgres.pendingRelay)
}
...
object DualDatabases extends Logging {
  ...
  def runAsync[T](f: => T): Future[T] = Future {
    Try(f) match {
...
case class DualPendingRelayDb(sqlite: SqlitePendingRelayDb, postgres: PgPendingRelayDb) extends PendingRelayDb {
  override def addPendingRelay(channelId: ByteVector32, cmd: HtlcSettlementCommand): Unit = {
    runAsync(postgres.addPendingRelay(channelId, cmd))
    sqlite.addPendingRelay(channelId, cmd)
  }

  override def removePendingRelay(channelId: ByteVector32, htlcId: Long): Unit = {
    runAsync(postgres.removePendingRelay(channelId, htlcId))
    sqlite.removePendingRelay(channelId, htlcId)
  }

  override def listPendingRelay(channelId: ByteVector32): Seq[HtlcSettlementCommand] = {
    runAsync(postgres.listPendingRelay(channelId))
    sqlite.listPendingRelay(channelId)
  }

  override def listPendingRelay(): Set[(ByteVector32, Long)] = {
    runAsync(postgres.listPendingRelay())
    sqlite.listPendingRelay()
  }
